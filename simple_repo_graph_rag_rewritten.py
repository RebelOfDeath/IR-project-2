...
class AdaptiveCallGraphConfig:
    ...
    @classmethod
    def from_hydra_config(cls, cfg: "DictConfig") -> "AdaptiveCallGraphConfig":
        """Create an AdaptiveCallGraphConfig from a Hydra DictConfig."""
        r = cfg.retrieval
        return cls(
            query_window_lines=cfg.context.get("query_window", 120),
            bm25_topk=r.pool.get("bm25_topk", 200),
            initial_pool_size=r.pool.get("initial_pool_size", 80),
            frontier_pool_size=r.pool.get("frontier_pool_size", 120),
            batch_size=r.pool.get("batch_size", 6),
            top_seed_count=r.pool.get("top_seed_count", 8),
            max_selected_entities=r.pool.get("max_selected_entities", 48),
            max_entities_per_file=r.pool.get("max_entities_per_file", 4),
            max_files=cfg.context.get("max_files", 6),
            max_context_tokens=cfg.context.get("max_tokens", 7000),
            max_entity_lines=cfg.context.get("max_entity_lines", 80),
            snippet_padding_lines=cfg.context.get("snippet_padding_lines", 2),
            min_lines=cfg.context.get("min_lines", 3),
            max_hops=r.graph.get("max_hop", 2),
            hop_decay=r.graph.get("hop_decay", 0.65),
            seed_temperature=r.graph.get("seed_temperature", 4.0),
            bm25_weight=r.scoring.get("bm25_weight", 0.45),
            symbol_weight=r.scoring.get("symbol_weight", 0.18),
            file_prior_weight=r.scoring.get("file_prior_weight", 0.12),
            modified_bonus=r.scoring.get("modified_bonus", 0.10),
            same_file_bonus=r.scoring.get("same_file_bonus", 0.06),
            graph_affinity_weight=r.scoring.get("graph_affinity_weight", 0.25),
            import_weight=r.graph.get("import_weight", 0.30),
            reverse_import_weight=r.graph.get("reverse_import_weight", 0.12),
            call_weight=r.graph.get("call_weight", 1.00),
            reverse_call_weight=r.graph.get("reverse_call_weight", 0.75),
            inheritance_weight=r.graph.get("inheritance_weight", 0.55),
            type_ref_weight=r.graph.get("type_ref_weight", 0.40),
            sibling_weight=r.graph.get("sibling_weight", 0.10),
            enable_slidegar_hook=r.get("slidegar", {}).get("enabled", False),
            slide_window_size=r.get("slidegar", {}).get("window_size", 20),
            slide_step_size=r.get("slidegar", {}).get("step_size", 10),
            extraction_cache_dir=cfg.get("cache_dir", None),
        )
...
def main_argparse() -> None:
    """Original argparse-based entry point."""
    args = parse_args()
...
def _build_prediction_filename(cfg: "DictConfig") -> str:
    """Build a descriptive prediction filename from Hydra config."""
    ...
    ga = cfg.retrieval.scoring.graph_affinity_weight
    parts.append(f"bm{bm}-ga{ga}")
    return "_".join(str(p) for p in parts) + ".jsonl"


def run_with_config(cfg: "DictConfig", original_cwd: str) -> Optional[Dict[str, float]]:
    """Run the adaptive call-graph pipeline with the given Hydra config."""
    print(OmegaConf.to_yaml(cfg))

    config = AdaptiveCallGraphConfig.from_hydra_config(cfg)
    language = cfg.data.lang
    stage = cfg.data.stage

    print(f"\nRunning AdaptiveCallGraphRAG for stage '{stage}', language '{language}'")
    print(f"Config: max_hops={config.max_hops}, bm25={config.bm25_weight}, "
          f"graph_affinity={config.graph_affinity_weight}, symbol={config.symbol_weight}")
    ...
    db_path = os.path.join(original_cwd, "experiments.db") if ExperimentDB is not None else None
    ...
    eval_results = process_tasks(
        ...
        language=language,
        config=config,
        evaluate_results=cfg.evaluation.get("enabled", False),
        data_dir_for_eval=data_dir if cfg.evaluation.get("enabled", False) else None,
        ollama_url=cfg.evaluation.get("ollama_url", "http://localhost:11434"),
        model=cfg.evaluation.get("model", "JetBrains/Mellum-4b-sft-python"),
...
def main_compose() -> None:
    """
    Hydra compose API entry point.

    Usage:
        python simple_repo_graph_rag_rewritten.py retrieval=adaptive_call_graph
        python simple_repo_graph_rag_rewritten.py retrieval=adaptive_call_graph retrieval.graph.max_hop=3
        python simple_repo_graph_rag_rewritten.py --multirun retrieval.graph.max_hop=1,2,3
    """
    ...
    # Default to adaptive_call_graph retrieval if no retrieval override is given
    if not any(o.startswith("retrieval=") for o in overrides):
        overrides.insert(0, "retrieval=adaptive_call_graph")
